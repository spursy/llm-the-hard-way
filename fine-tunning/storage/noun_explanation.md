...
```
CheckPoint（检查点）是计算机科学、工程、游戏开发领域中广泛使用的技术、用于记录系统或程序的当前轧辊台，以便在后续恢复、调试或容错时快速还原。其核心目标是保障数据一致性、容错性和可恢复性，同时优化资源使用
```
...
```
CheckPoint 是一段预先定义的代码或机制，用于在特定时刻（如周期性、事件触发或手动操作）保存系统状态、变量值、内存数据等关键信息到持久化存储（如磁盘、SSD）
```

**与 "保存/加载" 的区别**

- CheckPoint: 通常用于自动化、系统级或后台任务（如数据库、虚拟机）
- Save/Load: 多用于用户交互场景（如游戏存档、文档保存）
...
- 在事务处理中，CheckPoint 将内存中的未提交数据强制写入磁盘，确保崩溃后能恢复到一致状态

**文件系统**

- 结合 COW（如 ZFS），CheckPoint 可创建快找，记录数据变更的差异块

#### 容错与恢复

**虚拟机/容器**

- 保存虚拟机内存、磁盘状态，允许在系统崩溃或故障后快速恢复

**分布式系统**

- 在 Kafka、Hadoop 等系统中，CheckPoint 用于跟踪消费者/任务的进度，避免重启后重复处理数据
...
- 触发：按照时间间隔、事务量或手动触发
- FLUSH: 将缓冲区数据写入磁盘
- 记录位置：在日志中标记 CheckPoint 的结束位置

#### 虚拟机 CheckPoint

**原理**

- 内存快照：通过 VMM（虚拟机监控器）保存虚拟机内存内容
- 磁盘差异：使用 COW 技术记录磁盘增量变化

**性能优化**

- 增量备份：仅保存自上次 CheckPoint 后的改动

#### 分布式系统的 CheckPoint

**消费者 Offset 保存（Kafka）**

- 消费者在处理消息后，将已读位移量提交到 Zookeeper/RocksDB
- 恢复时从提交的 Offset 继续消费
